...
import { refineResumeWithInterviewFeedback } from "@/services/ai/resumes/ai"
...
export async function POST(req: Request) {
  ...
  if (!resumeFile || !jobInfoId || !interviewId) {
  ...
  if (!interview.feedback) {
    return new Response(
      "Interview feedback not yet generated",
      { status: 400 }
    )
  }
  ...
  const res = await refineResumeWithInterviewFeedback({
    resumeFile,
    jobInfo,
