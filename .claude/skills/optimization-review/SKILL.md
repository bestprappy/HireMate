...
description: "Review HireMate code for performance and efficiency issues across Next.js client and Golang Fiber services, then provide prioritized, low-risk optimization recommendations."
---
...
## HireMate Optimization Rules To Enforce

1. Respect architecture constraints (no cross-schema joins, no shared DTO coupling).
2. Keep Go 1.23+ and Fiber v2 compatibility intact for all optimization-related dependency decisions.
...
3. External Calls:
   - Missing context timeouts/circuit-breaker/retry strategy.
4. Events:
   - Inefficient outbox polling/dispatch loops.
   - Duplicate event handling risks and missing idempotency checks.
5. Caching:
   - Candidate read paths for Redis or in-memory caching.
